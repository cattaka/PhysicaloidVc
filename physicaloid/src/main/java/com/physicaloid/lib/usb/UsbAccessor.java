...
public enum UsbAccessor {
    ...
    public UsbDevice device(int devNum) {
        ...
        for (UsbDevice device : mManager.getDeviceList().values()) {
            if (DEBUG_SHOW) {
                Log.d(TAG, "Device(" + count + ") : " + device.toString());
            }
            ...
            if (count == devNum) {
                ...
                if (!mManager.hasPermission(device)) {
                    if (DEBUG_SHOW) {
                        Log.d(TAG, "Doesn't have permission device(" + count + ") : " + device.toString());
                    }
                    return null;
                } else {
                    return device;
        ...
        if (DEBUG_SHOW) {
            Log.d(TAG, "Cannot find device(" + devNum + ")");
        }
        return null;
    }
    ...
    public UsbInterface intface(int devNum, int intfNum) {
        ...
        for (int i = 0; i < devCount; i++) {
            if (i == intfNum) {
                if (DEBUG_SHOW) {
                    Log.d(TAG, "Interface(" + devNum + "," + intfNum + ") : " + dev.getInterface(i).toString());
                }
                return dev.getInterface(i);
            }
        }
        if (DEBUG_SHOW) {
            Log.d(TAG, "Cannot find interface(" + devNum + "," + intfNum + ")");
        }
        return null;
    }
    ...
    public UsbEndpoint endpoint(int devNum, int intfNum, int epNum) {
        ...
        for (int i = 0; i < epCount; i++) {
            if (i == epNum) {
                if (DEBUG_SHOW) {
                    Log.d(TAG, "Endpoint(" + devNum + "," + intfNum + "," + epNum + ") : " + intf.getEndpoint(i).toString());
                }
                return intf.getEndpoint(i);
            }
        }
        if (DEBUG_SHOW) {
            Log.d(TAG, "Cannot find endpoint(" + devNum + "," + intfNum + "," + epNum + ")");
        }
        return null;
    }
    ...
    public boolean openDevice(int devNum, int intfNum, int ch) {
        ...
        if (con.claimInterface(intface(devNum, intfNum), true)) {
            ...
            if (DEBUG_SHOW) {
                Log.d(TAG, "Cannot get claim interface(" + devNum + "," + intfNum + ")");
            }
    ...
    public boolean closeAll() {
        ...
        for (int i = 0; i < mConnection.size(); i++) {
            ...
            if (DEBUG_SHOW) {
                Log.d(TAG, "Close(" + mConnection.keyAt(i) + ")");
            }
            con.close();
            ret = true;
        }
